...
export async function updateScriptContent(
  ...
  try {
    ...
    console.error('Apps Script API Error:', JSON.stringify(error, Object.getOwnPropertyNames(error), 2));
 ...
 * This is a convenience function that gets all files, updates one, and saves back
 */
export async function updateScriptFile(
  ...
  // Find and update the file, or add it if it doesn't exist
  let fileFound = false;
